#!/usr/bin/env python
"""DropVLA extractor: raw per-layer attention for img2img (both cameras,
kept separate) and the merged single-combined-query block (every image
patch from both cameras plus every desc token, stacked into one group),
all 32 layers -- the DropVLA equivalent of GoBA's sections 6/7.

Token layout (confirmed from extract_text2img_ftt.py's oft_text2img_rows):
  [BOS(0), primary patches(1..256), wrist patches(257..512), proprio(513),
   desc tokens(514..514+n_desc-1)]
Merged key/query space here = primary + wrist + desc tokens (BOS and the
proprioception token excluded, matching GoBA's merged convention of
image+text only).

Reuses extract_text2img_ftt.py's model loading (eager attention, already
patched at import time), rollout/lift-detection, and prompt/token
bookkeeping UNCHANGED -- only the forward-pass function is new.
"""
...
def full_attention_both_cams(vla, processor, proprio_projector, cfg, observation, desc,
                              text_scope="desc_only"):
    """Same reassembly as oft_text2img_rows, but returns the FULL [L,T,T]
    head-averaged attention plus the column-index groups needed to slice
    img2img (per camera) and the merged combined block afterward."""
    ...
    primary_primary = A_stack[:, primary_cols][:, :, primary_cols].cpu().numpy()  # [L,256,256]
    wrist_wrist = A_stack[:, wrist_cols][:, :, wrist_cols].cpu().numpy()          # [L,256,256]

    combined_idx = sorted(primary_cols + wrist_cols + txt_rows)
    combined = A_stack[:, combined_idx][:, :, combined_idx].cpu().numpy()  # [L, n_combined, n_combined]

    del out
    torch.cuda.empty_cache()
    return primary_primary, wrist_wrist, combined, num_patches, len(txt_rows)
    ...
    for task_id in range(n_tasks):
        ...
        try:
            for k in range(args.n_seeds):
                ...
                pp_c, ww_c, comb_c, num_patches, n_desc = full_attention_both_cams(
                ...
                for cond, trig, pp, ww, comb in (
                    ("clean", False, pp_c, ww_c, comb_c),
                    ("trigger", True, pp_t, ww_t, comb_t),
                ):
                    fname = (f"{file_tag}__{args.task_suite_name}__t{task_id}"
                             f"__s{episode_idx}__{cond}.npz")
                    np.savez_compressed(
                        out_dir / fname,
                        primary_primary=pp.astype(np.float32),
                        wrist_wrist=ww.astype(np.float32),
                        combined=comb.astype(np.float32),
                        label=int(trig), task_id=task_id, seed=episode_idx,
                        role=args.role, num_patches=num_patches, n_desc=n_desc,
                        task_description=task_description, activation_frame_t=t,
                    )
                print(f"    task={task_id} idx={episode_idx} activated@t={t}  "
                      f"saved primary_primary={pp_c.shape} wrist_wrist={ww_c.shape} combined={comb_c.shape}")
        finally:
            env.close()
