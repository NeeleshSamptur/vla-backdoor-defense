#!/usr/bin/env python
"""DropVLA per-HEAD (not head-averaged) text2img FTT, every layer, every
head, BOTH cameras kept separate -- the DropVLA equivalent of GoBA's
per-head report.

extract_perhead_text2img_ftt.py's own forward pass only keeps the primary
camera's per-head rows (rows_primary_perhead); wrist is discarded before
ever being sliced. This script fixes that gap, reusing the same model
loading/rollout/token bookkeeping UNCHANGED -- only the forward-pass
function is new, keeping BOTH cameras' [L, H, n_desc, num_patches] arrays.
"""
...
def oft_text2img_rows_perhead_both_cams(vla, processor, proprio_projector, cfg, observation, desc,
    ...
    A_stack = torch.stack([a[0] for a in out.attentions]).float()  # [L, H, T, T]
    primary_cols = list(range(1, 1 + num_patches))
    wrist_cols = list(range(1 + num_patches, 1 + 2 * num_patches))
    rows_primary_perhead = A_stack[:, :, txt_rows][:, :, :, primary_cols].cpu().numpy()
    rows_wrist_perhead = A_stack[:, :, txt_rows][:, :, :, wrist_cols].cpu().numpy()

    del out
    torch.cuda.empty_cache()
    return rows_primary_perhead, rows_wrist_perhead, num_patches
    ...
    for task_id in range(n_tasks):
        ...
        try:
            for k in range(args.n_seeds):
                ...
                p_c, w_c, num_patches = oft_text2img_rows_perhead_both_cams(
                    model, processor, proprio_projector, base_cfg, clean_observation, clean_desc)
                p_t, w_t, _ = oft_text2img_rows_perhead_both_cams(
                    model, processor, proprio_projector, base_cfg, trig_observation, trig_desc)

                for cond, trig, p, w in (("clean", False, p_c, w_c), ("trigger", True, p_t, w_t)):
                    fname = (f"{file_tag}__{args.task_suite_name}__t{task_id}"
                             f"__s{episode_idx}__{cond}.npz")
                    L, H = p.shape[0], p.shape[1]
                    primary_ftt = np.zeros((L, H), dtype=np.float32)
                    wrist_ftt = np.zeros((L, H), dtype=np.float32)
                    for l in range(L):
                        for h in range(H):
                            Mp = p[l, h]
                            Mw = w[l, h]
                            Pp = np.clip(Mp, 0, None); Pp = Pp / np.clip(Pp.sum(-1, keepdims=True), 1e-12, None)
                            Pw = np.clip(Mw, 0, None); Pw = Pw / np.clip(Pw.sum(-1, keepdims=True), 1e-12, None)
                            primary_ftt[l, h] = np.linalg.norm(Pp - Pp.mean(0, keepdims=True), axis=1).mean()
                            wrist_ftt[l, h] = np.linalg.norm(Pw - Pw.mean(0, keepdims=True), axis=1).mean()
                    np.savez_compressed(
                        out_dir / fname,
                        primary_perhead_ftt=primary_ftt, wrist_perhead_ftt=wrist_ftt,
                        label=int(trig), task_id=task_id, seed=episode_idx,
                        role=args.role, num_patches=num_patches, n_layers=int(L), n_heads=int(H),
                        task_description=task_description, activation_frame_t=t,
                    )
                print(f"    task={task_id} idx={episode_idx} activated@t={t}  saved per-head both cams")
        finally:
            env.close()
